...
for i, antcnt in enumerate(anticentroids):
    print(f"Антицентроид {i+1}: Сумма расстояний = {antcnt[0]} x = {antcnt[1][0]} y = {antcnt[1][1]}")

ans_antic = max(anticentroids, key=lambda x: x[0])

print(f"Искомый антицентроид: x = dist = {ans_antic[0]}, x = {ans_antic[1][0]}, y = {ans_antic[1][1]}" )
